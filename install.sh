...
(cd "$ROOT" && "$VENV/bin/python" -m compileall -q agentairgap tests project_gui.py && "$VENV/bin/python" -m unittest discover -s tests -p 'test_*.py' -v)
echo "AgentAirgap installation verified"
